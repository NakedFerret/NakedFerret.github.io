...
function prepareIndexedDBDemo() {
    ...
    var people = [new Person("Mary"), new Person("Bob"), new Person("Phil")];
	...
	var peopleOBJ = db.createObjectStore(peopleObjectStoreName, {keyPath: "id", autoIncrement: true});
	peopleOBJ.createIndex("name", "name", {unique: false});

	peopleOBJ.transaction.oncomplete = function(event) {
	    db.close();
	};
    };
	...
	peopleOBJ.get(person.id).onsuccess = function () {
    ...
    document.querySelector("#cursor-db").addEventListener('click', function() {
	...
	var peopleCount = 0;
	peopleOBJ.openCursor().onsuccess = function() {
		...
		utils.status.show("Found " + peopleCount + " people");
	    }
	};
    });

    document.querySelector("#index-db").addEventListener('click', function() {
	...
	peopleOBJ.index("name").get(person.name).onsuccess = function() {
